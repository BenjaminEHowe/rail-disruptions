...
- `S3_ENDPOINT`: the S3-compatible object storage endpoint.
- `S3_ACCESS_KEY`: the access key to use for S3.
- `S3_SECRET_KEY`: the secret key to use for S3.
- `S3_BUCKET`: the bucket to store the objects in (must already exist).
...
- `S3_SECURE`: whether to use a secure connection when connecting to S3, defaults to `1`.
