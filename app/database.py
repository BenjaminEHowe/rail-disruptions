...
def select_incident_version_where_incident_id_and_updated(incident_id: str, updated_ts: datetime, session: Session = get_session()) -> Optional[IncidentVersion]:
  with session:
    return session.query(IncidentVersion).where(IncidentVersion.incident_id == incident_id, IncidentVersion.updated_ts == updated_ts).first()
